...
class Aggus(nn.Module):
    ...
    def forward(self, layer1, layer2, layer3, layer4):
        output=[layer1] 
        shapex, shapey = layer1.shape[2], layer1.shape[3]
        for l in [layer2, layer3,layer4]:
            output.append(F.interpolate(l,(shapex,shapey)))
...
class ExtremeResNet(nn.Module):
    ...
    def forward(self,x):
        ...
        output = self.aggus(*[v for k, v in feats.items()])
        output=self.downsample(output)
        x=torch.cat((feats["layer4"],output),dim=1)
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        x = self.fc(x)
