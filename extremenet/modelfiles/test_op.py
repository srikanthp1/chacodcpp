...
# layerOne.size(0), layerOne.size(1)
    ...
    torch::Tensor aggupscop(torch::Tensor layerOne, torch::Tensor layerTwo, torch::Tensor layerThree, torch::Tensor layerFour) {
        ...
        auto layer2 = torch::nn::functional::interpolate(layerTwo, torch::nn::functional::InterpolateFuncOptions().size(std::vector<int64_t>({layerOne.size(0), layerOne.size(1)})).mode(torch::kBilinear));
        auto layer3 = torch::nn::functional::interpolate(layerThree, torch::nn::functional::InterpolateFuncOptions().size(std::vector<int64_t>({layerOne.size(0), layerOne.size(1)})).mode(torch::kBilinear));
        auto layer4 = torch::nn::functional::interpolate(layerFour, torch::nn::functional::InterpolateFuncOptions().size(std::vector<int64_t>({layerOne.size(0), layerOne.size(1)})).mode(torch::kBilinear));
